 ...
 * @example
 * // 查询英语（默认语言）
 * buildLocaleCondition("en") // { locale: "en" }
 *
 * // 查询中文（非默认语言，需要回退）
 * buildLocaleCondition("zh") // { OR: [{ locale: "zh" }, { locale: "en" }] }
 */
 ...
 * @returns 翻译对象或 undefined
 *
 * @example
 * const translation = getTranslationWithFallback(game.translations, "zh")
 ...
 *
 * @example
 * const title = getTranslatedField(game.translations, "zh", "title", "Untitled")
 */
 ...
 *
 * @example
 * const { title, description } = getTranslatedFields(game.translations, "zh", ["title", "description"])
 */
