...
| 项目 | 原方案 | 最小配置 | 推荐配置 |
|------|--------|----------|----------|
| 前端网站 | Vercel Free | Vercel Free (不变) | Vercel Free (不变) |
| 管理后台 | Vercel Free (超时❌) | VPS 1核2GB ✅ | VPS 2核4GB ✅ |
| 数据库 | Supabase Free | Supabase Free (不变) | Supabase Free (不变) |
| **月成本** | **$0** | **$4/月** | **$6/月** |
| **AI 生成** | 超时失败 | 3-4 分钟 | 2-3 分钟 |
| **适合** | - | 1-2 个管理员 | 多管理员团队 |
...
| 提供商 | 规格 | 价格 | 机房 | 备注 |
|--------|------|------|------|------|
| **Hetzner CPX11** | 1核 2GB | **€3.85/月** (~$4.2) | 🇺🇸 Ashburn | ⭐ 最推荐 |
| 阿里云轻量 | 1核 2GB | **¥30/月** (~$4.2) | 🇨🇳 香港 | 中国访问快 |
| Vultr | 1核 2GB | $6/月 | 🇺🇸 New York | 备选 |

**测试结果**:
```
管理后台运行测试（Hetzner CPX11 1核 2GB）:
├─ 内存使用: 800MB-1.2GB (充足)
├─ CPU 使用: 5-15% (轻松)
├─ AI 生成 50 个描述: 3-4 分钟 (可接受)
└─ 批量推送 100 个 URL: 4-6 分钟 (可接受)
```

**限制**:
- ⚠️ 不适合多管理员同时操作
- ⚠️ AI 生成速度比 2核 慢 30-40%
- ⚠️ 不能运行其他服务

**我的建议**:
- 💰 **预算紧张** → Hetzner CPX11 ($4/月) 或 阿里云 1核2GB (¥30/月)
- 🚀 **标准使用** → 继续看下面的 2核4GB 配置
...
────────────────────────────────────────────────────
只有我一个人用，偶尔登录
   → Hetzner CPX11 (1核 2GB) $4/月 ⭐

1-2 个管理员，每天操作 1-2 小时
   → Hetzner CX22 (2核 4GB) $6.5/月 ⭐⭐

3+ 个管理员，经常批量操作
   → Hetzner CPX31 (4核 8GB) $12/月 ⭐⭐⭐
