    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          let dbConfig = allConfigs.find(c => c.id === configId && c.isEnabled)
          ...
          sendComplete({
            ...
            citations: filteredWebsites.map(f => ({
              title: f.title,
              url: f.url,
              confidence: (f as any).confidence,  // 如果有筛选信息
              reasoning: (f as any).reasoning     // 如果有筛选理由
            })),
            statistics: {
              duration,
              urlsFiltered: searchResults.length - filteredWebsites.length,  // 被筛掉的数量
              ...statistics
            }
          })
